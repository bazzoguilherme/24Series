package Tests.Constantes;

public final class Constantes_Series {
    public static final char IDENTIFICADOR_SERIE = 'S';

    public static final String NOME_SERIE1 = "Mr. Robot";
    public static final String GENERO_SERIE1_1 = "Drama";
    public static final String GENERO_SERIE1_2 = "Suspense Psicologico";
    public static final int DURACAO_SERIE1 = 65;
    public static final String PRODUTORA_SERIE1 = "UCP";
    public static final String DIRETOR_SERIE1 = "Sam Esmail";
    public static final int ANO_SERIE1 = 2015;
    public static final int EP_SERIE1_TEMP1 = 10;
    public static final int EP_SERIE1_TEMP2 = 12;
    public static final int EP_SERIE1_TEMP3 = 10;

    
    public static final String NOME_SERIE2 = "Haikyuu";
    public static final String GENERO_SERIE2_1 = "Anime";
    public static final String GENERO_SERIE2_2 = "Esporte";
    public static final int DURACAO_SERIE2 = 30;
    public static final String PRODUTORA_SERIE2 = "MBS";
    public static final String DIRETOR_SERIE2 = "Susumu Mitsunaka";
    public static final int ANO_SERIE2 = 2014;
    public static final int EP_SERIE2_TEMP1 = 25;
    public static final int EP_SERIE2_TEMP2 = 25;
    public static final int EP_SERIE2_TEMP3 = 10;
    
    public static final String NOME_SERIE3 = "The Walking Dead";
    public static final String GENERO_SERIE3_1 = "Drama";
    public static final int DURACAO_SERIE3 = 40;
    public static final String PRODUTORA_SERIE3 = "Fox";
    public static final String DIRETOR_SERIE3 = "Jolly Dale";
    public static final int ANO_SERIE3 = 2010;
    public static final int EP_SERIE3_TEMP1 = 22;
    public static final int EP_SERIE3_TEMP2 = 25;
    public static final int EP_SERIE3_TEMP3 = 26;
}
